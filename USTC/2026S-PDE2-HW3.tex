...
\item [(1)] 若 $b_1=0$. 证明如下结论。
	...
	\item [(1c)] 对$R>0$，记$\Om_R=\Om\cap B(\bd{0},R)$，则$\{u^\eps\}$ 存在子列在 $L^\infty(0,T;L^2(\Om_R))$ 中强收敛，且 $\sup\limits_{t\in[0,T]}\|u^\eps-u\|_{L^2(\Om)}\leqslant C\sqrt{\eps}$. 
	\end{itemize}
\item [(2)] 若 $b_1=1$，证明：$\sup\limits_{t\in[0,T]}\|u^\eps\|_{L^2(\Om)}$ 关于 $\eps$ 仍然一致有界，并问：$\sup\limits_{t\in[0,T]}\|u^\eps\|_{H^1(\Om)}$ 关于 $\eps$ 是否一致有界？证明结论或者构造反例。
