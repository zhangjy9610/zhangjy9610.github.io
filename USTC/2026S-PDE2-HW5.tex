...
\p_t^2 u -\lap  u +f( u )=0&~~t>0,~\xx\in\R^d;\\
 u (0,\xx)= u_0(\xx),~\p_t  u (0,x)=\psi_1(\xx)&~~\xx\in\R^d.
\end{cases}
...
\item [(4)] 证明 (3) 对于拟线性波动方程 $\p_t^2  u  -\lap  u  + f( u ,\pb  u )=0$ 的光滑解 $ u $ 也成立，其中 $f$ 连续且 $f(0,\mathbf{0})=0$. 
\end{itemize}
...
\item [(2)] 令 $I(t):=\int_{\R^3} u^2\dxx$，证明：$I''(t)\geqslant 6\int_{\R^3}(\p_t u)^2\dxx-8E(0)$，进而存在 $t_1\geqslant _0$ 使得对任意 $t\geqslant t_1$ 有 $I'(t)>0$.
